...
@section('content')
    ...
    <div class="container mt-5">
        <a href="{{ route('admin/doctors/create') }}" class="btn btn-primary rounded-pill"><i
                class="fa fa-user-plus fa-md"></i> Add Doctor</a>
        <a href="{{ route('admin/doctors/trash') }}" class="btn btn-danger rounded-pill"><i class="fa fa-trash"
                                ...
                                @forelse ($doctors as $doctor)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <img src="{{ asset('/storage/' . $doctor->profile_image) }}" class="rounded"
                                                style="width: 50px">
                                        </td>
                                        ...
                                        <td class="text-center">{{ $doctor->date_of_birth }}</td>
                                        <td class="text-center">{{ $doctor->amount }}</td>
                                        <td class="text-center">
                                            <a href="{{ route('admin/doctors/edit', ['id' => $doctor->id]) }}"
                                                class="btn btn-warning rounded-pill">Edit</a>
                                            <a onclick="confirmDelete(this)"
                                                data-url="{{ route('admin/doctors/delete', ['id' => $doctor->id]) }}"
                                                class="btn btn-danger rounded-pill"><i class="fa fa-eraser"
